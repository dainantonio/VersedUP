...
function HomeView({ onNew, onLibrary, onContinue, onReflectVerseOfDay, hasActive, streak, displayName, devotionals, onOpen, onQuickStart }) {
  const [quickText, setQuickText] = useState("");
  const inputRef = useRef(null);

  const handleSubmit = () => {
    if (onQuickStart && quickText.trim()) {
      onQuickStart(quickText.trim());
    } else {
      onNew();
    }
    setQuickText("");
  ...
  const lastDraft = devotionals.length > 0 ? devotionals[devotionals.length - 1] : null;

  const ago = lastDraft ? (() => {
    const diff = Date.now() - new Date(lastDraft.updatedAt || lastDraft.createdAt || Date.now()).getTime();
  ...
  return (
    <div className="-mx-4 -mt-4 min-h-[calc(100svh-56px)] flex flex-col bg-black text-white pb-24 animate-enter">
        ...
        <div className="flex items-center gap-2">
          {streak.count > 0 && (
            <div className="flex items-center gap-1.5 bg-white/10 rounded-full px-3 py-1.5">
              ...
              <span className="text-xs font-bold text-white/70">{streak.count} day streak</span>
            </div>
          )}
          {hasActive && (
            <button
              type="button"
              onClick={onContinue}
              className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 rounded-full px-3 py-1.5 text-xs font-bold text-white/70 transition-all"
            >
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-6 gap-5">
        {/* Logo circle */}
        <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-2xl ring-4 ring-white/10">
          <img
        ...
        <div className="space-y-1">
          <h1 className="text-[2rem] font-black text-white tracking-tight leading-none">VersedUP</h1>
          <p className="text-sm text-white/40 font-medium">by {displayName || "dain russell"}</p>
        </div>

        {/* Tagline */}
        <p className="text-base text-white/60 max-w-[260px] leading-relaxed font-medium">
          Faith-based creative assistant for devotionals and Christian content
        </p>
        ...
        <div className="flex flex-col items-center gap-2.5 mt-2">
          <button
            ...
            className="flex items-center gap-2 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-400/20 rounded-full px-5 py-2.5 text-sm font-semibold text-emerald-300 transition-all active:scale-95"
          >
            <Sparkles className="w-3.5 h-3.5" />
            {VERSE_OF_DAY.verseRef} &middot; Reflect today
          </button>

          {lastDraft && (
            <button
              type="button"
              onClick={() => onOpen(lastDraft)}
              className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full px-5 py-2.5 text-xs font-semibold text-white/50 transition-all active:scale-95 max-w-[280px]"
            >
              <Clock className="w-3 h-3 flex-shrink-0" />
              <span className="truncate">{lastDraft.verseRef || "Recent draft"}</span>
              {ago && <span className="text-white/30 flex-shrink-0">&nbsp;&middot; {ago}</span>}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="px-5 pb-6">
        <div className="flex items-center gap-3 bg-white/10 border border-white/10 rounded-2xl px-4 py-3.5 focus-within:border-white/30 transition-all">
          <button
            type="button"
            onClick={onNew}
            className="w-7 h-7 rounded-full bg-white/15 hover:bg-white/25 flex items-center justify-center flex-shrink-0 transition-all"
            title="New entry"
          >
          ...
          <input
            ...
            placeholder="Start with a verse or thought…"
            className="flex-1 bg-transparent text-white placeholder-white/30 outline-none text-sm font-medium"
          />
          <button
            type="button"
            onClick={handleSubmit}
            className="w-8 h-8 rounded-full bg-white flex items-center justify-center flex-shrink-0 shadow-lg hover:scale-110 active:scale-95 transition-all"
            title="Draft for me"
          >
...
${devotional.reflection}`;
  ...
  return (
    <div className="space-y-5 pb-20 animate-enter">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-xl font-extrabold text-slate-900 leading-tight">
            {devotional.title ? devotional.title : "New Entry"}
          </div>
      ...
      <div className="bg-white rounded-[1.75rem] border border-slate-100 shadow-sm p-5 space-y-6">
          <div className="space-y-4">
    ...
    <div className="space-y-5 pb-20 animate-enter">
      ...
      <div className="flex items-start justify-between gap-3 pt-1">
        <div>
          ...
          <div className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">Review and refine before sharing</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[1.75rem] border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-5 border-b border-slate-100">
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Scripture</div>
          <div className="text-xs font-extrabold text-emerald-700 uppercase tracking-wide mb-2">{devotional.verseRef || "—"}</div>
          <div className="text-base whitespace-pre-wrap text-slate-800 font-serif-scripture leading-relaxed italic">
            {devotional.verseText ? `"${devotional.verseText}"` : "—"}
          </div>
        </div>

        {/* Reflection block */}
        <div className={`px-6 py-5 ${(devotional.prayer || devotional.questions) ? "border-b border-slate-100" : ""}`}>
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Reflection</div>
          <div className="text-sm whitespace-pre-wrap text-slate-800 leading-relaxed font-medium">{devotional.reflection || "—"}</div>
        </div>

        {/* Prayer block */}
        {!!devotional.prayer && (
          <div className={`px-6 py-5 bg-sky-50/40 ${devotional.questions ? "border-b border-slate-100" : ""}`}>
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Prayer</div>
            <div className="text-sm whitespace-pre-wrap text-slate-700 italic leading-relaxed">{devotional.prayer}</div>
          </div>
        )}
        ...
        {!!devotional.questions && (
          <div className="px-6 py-5 bg-amber-50/40">
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Questions to Sit With</div>
            <div className="text-sm whitespace-pre-wrap text-slate-800 leading-relaxed">{devotional.questions}</div>
          </div>
        )}
      </div>
...
function CompileView({ devotional, settings, onUpdate, onBackToWrite }) {
  ...
  return (
    <div className="space-y-5 pb-24 animate-enter">
      <div>
        ...
        {slide === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center animate-enter space-y-6">
            <div className="flex flex-col items-center">
              ...
              />
              <div style={{display:"none"}} className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-emerald-400 to-teal-500 items-center justify-center shadow-2xl shadow-emerald-200">
