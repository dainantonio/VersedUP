...
const STORAGE_ANALYTICS_EVENTS = `${APP_ID}_analytics_events`;
...
function trackEvent(name, meta = {}) {
  try {
    const payload = { name, at: new Date().toISOString(), ...meta };
    const raw = localStorage.getItem(STORAGE_ANALYTICS_EVENTS);
    const parsed = safeParseJson(raw, []);
    const list = Array.isArray(parsed) ? parsed : [];
    list.push(payload);
    localStorage.setItem(STORAGE_ANALYTICS_EVENTS, JSON.stringify(list.slice(-200)));
  } catch {
  ...
  return (
    <div className="space-y-4 pb-20 animate-enter">
      ...
      <Card className={`border p-4 space-y-3 ${todaysAction.tone}`}>
        ...
        <RippleButton
          onClick={() => {
            const firstActionKey = `${APP_ID}_first_action_day`;
            const today = todayKey();
            if (localStorage.getItem(firstActionKey) !== today) {
              localStorage.setItem(firstActionKey, today);
              trackEvent("home_first_action", { source: "primary_cta" });
            }
            trackEvent("home_primary_cta_tap", { hasActive, hasLatestReady: Boolean(latestReady), hasLatest: Boolean(latest) });
...
function WriteView({ devotional, settings, onUpdate, onGoCompile, onGoPolish, onSaved, onFullscreenChange }) {
  ...
  const lastFullscreenToastState = useRef(null);
  useEffect(() => {
    const enabled = canvasFullscreen && step >= 2 && step <= 4;
    if (lastFullscreenToastState.current == null) {
      lastFullscreenToastState.current = enabled;
      return;
    }
    if (lastFullscreenToastState.current !== enabled) {
      pushToast(enabled ? "Focus mode enabled" : "Focus mode exited", 1200);
      lastFullscreenToastState.current = enabled;
    }
  }, [canvasFullscreen, step, pushToast]);
...
${devotional.reflection}`, txt);
  ...
  const getStepBlockReason = (nextStep) => {
    if (nextStep <= 1) return "";
    if (!verseReady) return "Add a verse reference first in Step 1.";
    if (nextStep === 4 && !heartReady) return "Add at least one reflection, prayer, or question in Step 2.";
    return "";
  };
  const goToStep = (nextStep) => {
    if (!canAccessStep(nextStep)) {
      const reason = getStepBlockReason(nextStep);
      if (reason) pushToast(reason, 1500);
      trackEvent("write_step_blocked", { fromStep: step, targetStep: nextStep, reason: reason || "unknown" });
      return;
    }
    if (nextStep === step) return;
    setStep(nextStep);
    trackEvent("write_step_changed", { fromStep: step, toStep: nextStep });
    pushToast(`Step ${nextStep}: ${stepTitles[nextStep - 1]}`, 1100);
  };
...
function AppInner({ session, starterMood, onLogout }) {
 ...
 // home | write | library | settings
  ...
  useEffect(() => {
    const onPopState = () => {
      if (writeFullscreenActive) {
        ...
        trackEvent("back_recovered", { context: "write_fullscreen" });
        return;
      }
      ...
      if (view === "write" && active) {
        ...
        if (currentStep > 1) {
          ...
          trackEvent("back_recovered", { context: "write_step", fromStep: currentStep, toStep: nextStep });
          return;
        }
      }
      ...
      if (view !== "home") {
        ...
        setView(previous === "settings" ? (lastNonSettingsView || "home") : previous);
        return;
      }
  ...
  const continueActiveFlow = () => {
    if (!active) {
      setView("home");
      return;
    }
    const stepKey = `${APP_ID}_wizard_step_${active.id}`;
    const savedStep = Number(localStorage.getItem(stepKey) || "0");
    const inferredStep = String(active.reflection || "").trim().length > 120
      ? 4
      : String(active.verseRef || "").trim()
        ? 2
        : 1;
    const stepToUse = [1, 2, 3, 4].includes(savedStep) ? savedStep : inferredStep;
    localStorage.setItem(stepKey, String(stepToUse));
    setView("write");
  };
...
Entries:\n${JSON.stringify(payload, null, 2)}`);
  ...
  return (
    <ToastContext.Provider value={{ pushToast }}>
      ...
      {/* ── Bottom Nav Bar ── */}
      {!writeFullscreenActive ? <BottomNav view={view} onHome={() => setView("home")} onWriteFromYourVerse={writeFromYourVerse} onContinueWrite={continueActiveFlow} onLibrary={() => setView("library")} onSettings={() => setView("settings")} showWriteHint={!hasUsedWriteFab && !customVerseRef} hasActiveDraft={Boolean(active)} /> : null}
    </div>
