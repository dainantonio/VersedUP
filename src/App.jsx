  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      {multiline ? (
        ...
        <input className={`w-full p-3 ${theme.appBg} border ${theme.border} rounded-xl focus:outline-none focus:ring-2 focus:ring-${theme.accent}-500/50 transition-all ${theme.textColor} font-medium ${theme.font}`} type="text" placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} />
      )}
    </div>
  );
};
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="relative py-2">
        ...
        <div className="relative flex justify-center"><span className={`${theme.appBg} px-2 text-xs font-semibold ${theme.subTextColor} uppercase tracking-wider`}>Or write your own</span></div>
      </div>
      ...
      <Button onClick={onSave} className="w-full" icon={Sparkles} themeKey={themeKey}>Save & Polish</Button>
    </div>
  );
};
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <Card className="min-h-[300px]" themeKey={themeKey}>
        <textarea className={`w-full h-full min-h-[300px] resize-none focus:outline-none ${theme.textColor} leading-relaxed bg-transparent ${theme.font}`} value={currentText} onChange={() => {}} readOnly />
      </Card>
    ...
    <div className="space-y-6 pb-24">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button onClick={onBack} className={`p-2 hover:${theme.primaryLight} rounded-full`}><ChevronLeft className={`w-6 h-6 ${theme.subTextColor}`} /></button>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
        {platforms.filter(p => settings.enabledPlatforms.includes(p.id)).map(p => {
          ...
          return (
            <button key={p.id} onClick={() => setActivePlatform(p.id)} className={`flex items-center gap-2 px-4 py-2 rounded-full whitespace-nowrap text-sm font-medium transition-colors ${isActive ? `${theme.primary} text-white` : `${theme.cardBg} border ${theme.border} ${theme.subTextColor}`}`}>
      ...
      <div className="px-1">
        <div className={`flex justify-between text-xs font-semibold ${theme.subTextColor} mb-1`}><span>{charCount} chars</span><span>Limit: {currentLimit}</span></div>
        <div className={`w-full h-2 ${theme.appBg} border ${theme.border} rounded-full overflow-hidden`}><div className={`h-full transition-all duration-500 ${isOverLimit ? 'bg-red-500' : theme.primary}`} style={{ width: `${Math.min((charCount / currentLimit) * 100, 100)}%` }} /></div>
        {isOverLimit && (
          <div className="mt-3 bg-red-50 border border-red-100 p-3 rounded-xl flex items-start gap-3 animate-in slide-in-from-top-1">
             ...
             <div className="flex-1"><p className="text-sm text-red-800 font-medium mb-1">Content too long.</p><button onClick={handleAutoFit} disabled={isFixing} className="text-xs bg-red-100 hover:bg-red-200 text-red-800 px-3 py-1.5 rounded-lg font-bold transition-colors flex items-center gap-1 mt-2">{isFixing ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />} Fix with AI</button></div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3"><Button variant="secondary" onClick={handleCopy} icon={Copy} themeKey={themeKey}>Copy Text</Button><Button variant="primary" onClick={handleShare} icon={Share2} disabled={isOverLimit} themeKey={themeKey}>Share / Post</Button></div>
    </div>
  );
};
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search..." className={`w-full pl-10 pr-4 py-3 ${theme.cardBg} border ${theme.border} rounded-xl focus:outline-none focus:ring-2 focus:ring-${theme.accent}-500/50 ${theme.textColor} ${theme.font}`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <button onClick={() => setShowFilter(!showFilter)} className={`p-3 rounded-xl border transition-colors ${statusFilter !== 'all' ? `${theme.primaryLight} ${theme.border} ${theme.primaryText}` : `${theme.cardBg} ${theme.border} ${theme.subTextColor}`}`}><Filter className="w-5 h-5" /></button>
      </div>
      {showFilter && (
        <div className={`flex gap-2 p-2 ${theme.appBg} rounded-xl overflow-x-auto no-scrollbar animate-in slide-in-from-top-2 border ${theme.border}`}>
          {['all', 'draft', 'polished', 'compiled'].map(status => (
            <button key={status} onClick={() => setStatusFilter(status)} className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors ${statusFilter === status ? `${theme.cardBg} ${theme.primaryText} shadow-sm` : `${theme.subTextColor} hover:${theme.textColor}`}`}>{status}</button>
          ))}
        </div>
      )}
      <div className="grid gap-3">
        {filtered.length === 0 ? <div className={`text-center py-12 ${theme.subTextColor}`}><Library className="w-12 h-12 mx-auto mb-3 opacity-20" /><p>No devotionals found.</p></div> : filtered.map(d => (
            <Card key={d.id} className="relative group" onClick={() => onSelect(d)} themeKey={themeKey}>
              <div className="flex justify-between items-start"><div><h3 className={`font-bold ${theme.textColor} ${theme.font}`}>{d.title || 'Untitled Devotional'}</h3><p className={`text-sm ${theme.primaryText} font-medium mt-1`}>{d.verseRef}</p></div><div className="flex items-center gap-2"><button onClick={(e) => { e.stopPropagation(); exportToDocument(d); }} className={`p-1.5 ${theme.subTextColor} hover:${theme.primaryText} hover:bg-slate-100 rounded-full transition-colors`} title="Export"><FileDown className="w-4 h-4" /></button><span className={`text-xs px-2 py-1 rounded-full ${d.status === 'compiled' ? `${theme.primaryLight} ${theme.primaryText}` : `${theme.appBg} ${theme.subTextColor}`}`}>{d.status}</span></div></div>
              <p className={`text-sm ${theme.subTextColor} mt-3 line-clamp-2`}>{d.rawText}</p>
              <div className={`flex justify-between items-center mt-4 pt-3 border-t ${theme.border}`}><span className={`text-xs ${theme.subTextColor}`}>{new Date(d.createdAt).toLocaleDateString()}</span><button onClick={(e) => { e.stopPropagation(); onDelete(d.id); }} className={`p-2 ${theme.subTextColor} hover:text-red-500 transition-colors`}><Trash2 className="w-4 h-4" /></button></div>
            </Card>
          ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <Card themeKey={themeKey}>
        ...
        <div className="grid grid-cols-3 gap-2">
          {['classic', 'sunrise', 'sunset'].map((key) => {
            const t = THEMES[key];
            return (
              <button key={key} onClick={() => updateSetting('theme', key)} className={`p-3 rounded-xl border text-center transition-all ${settings.theme === key ? `${t.primaryLight} ${t.primaryText} border-${t.accent}-200 ring-1 ring-${t.accent}-200` : `${theme.appBg} ${theme.border} ${theme.subTextColor}`}`}>
      ...
      <Card themeKey={themeKey}>
        ...
        <div className="space-y-4">
           <div className={`flex items-center justify-between p-3 ${theme.appBg} rounded-xl`}>
             <span className={`text-sm font-medium ${theme.textColor}`}>AI Provider</span>
             <select className={`p-2 bg-white rounded-lg text-sm border ${theme.border} ${theme.textColor}`} value={settings.aiProvider} onChange={(e) => updateSetting('aiProvider', e.target.value)}>
    ...
    <div className={`fixed bottom-0 left-0 right-0 ${theme.cardBg} border-t ${theme.border} pb-safe pt-2 px-6 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-40`}>
      <div className="flex justify-between items-end max-w-md mx-auto">
        {items.map(item => {
          ...
          if (item.prominent) return <button key={item.id} onClick={() => onChangeView(item.id)} className={`relative -top-5 ${theme.primary} text-white p-4 rounded-full shadow-lg shadow-${theme.accent}-500/30 hover:scale-105 transition-transform active:scale-95`}><Icon className="w-6 h-6" /></button>;
          return <button key={item.id} onClick={() => onChangeView(item.id)} className={`flex flex-col items-center gap-1 p-2 min-w-[64px] transition-colors ${isActive ? theme.primaryText : `${theme.subTextColor} hover:${theme.textColor}`}`}><Icon className={`w-6 h-6 ${isActive ? 'fill-current opacity-20' : ''}`} strokeWidth={isActive ? 2.5 : 2} /><span className={`text-[10px] font-medium ${theme.font}`}>{item.label}</span></button>;
        })}
      </div>
    </div>
  );
      ...
      <div className={`sticky top-0 z-30 ${theme.cardBg}/90 backdrop-blur-md border-b ${theme.border} px-4 py-3 transition-colors duration-300`}>
        <div onClick={() => setView('library')} className="flex flex-col items-center justify-center relative max-w-md mx-auto cursor-pointer hover:opacity-80 transition-opacity">
          <div className="flex items-center gap-3">
             <img src="Versedup logo 2.jpg" alt="VersedUP" className="w-10 h-10 object-contain rounded-full shadow-md bg-white" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
